...
object FacilitatorResponseParser {
    ...
    fun applyVNChanges(
        ...
        jealousyEnabled: Boolean = false, // Pass this from your modeSettings!
        ...
        for (chg in changes) {
            val slotIndex = idxFromSlotKey(chg.fromSlotKey) ?: continue
            val fromProfile = sessionProfile.slotRoster.getOrNull(slotIndex) ?: continue

            // Grab or create the relationship
        ...
        // --- 2. APPLY JEALOUSY PENALTY ---
        if (!jealousyEnabled) return
        ...
        for (g in gains) {
            ...
            val speaker = sessionProfile.slotRoster.getOrNull(fromIdx) ?: continue
            val speakerLoc = speaker.lastActiveLocation

            val targetKey = g.toSlotKey

            // Check everyone else in the roster
            sessionProfile.slotRoster.forEachIndexed { i, slot ->
                // Skip the person who just gained the points
                if (i == fromIdx) return@forEachIndexed

                // Skip if they aren't in the same room to see it happen!
                if (slot.lastActiveLocation != speakerLoc) return@forEachIndexed

                // Apply the Jealousy Penalty!
                val otherKey = slotKeyFromIndex(i)
                val rel2 = slot.vnRelationships.getOrPut(targetKey) {
                    ModeSettings.VNRelationship(
                ...
                Log.d("VN_Logic", "JEALOUSY: ${slot.name} lost $jealousyPenalty points with target $targetKey because they saw ${speaker.name} flirting!")
            }
        }
    }
