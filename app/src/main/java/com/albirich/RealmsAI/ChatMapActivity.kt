...
class ChatMapActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Load characters from the new Cache!
        ...
        // --- ADD THESE LINES: Load previous assignments so the map remembers them! ---
        val mapType = object : TypeToken<Map<String, String>>() {}.type

        intent.getStringExtra("CHARACTER_TO_AREA_JSON")?.let { json ->
            characterToAreaMap.putAll(Gson().fromJson(json, mapType))
        }

        intent.getStringExtra("CHARACTER_TO_LOCATION_JSON")?.let { json ->
            characterToLocationMap.putAll(Gson().fromJson(json, mapType))
        }
